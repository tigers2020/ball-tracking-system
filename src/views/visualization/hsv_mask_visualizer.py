...
class HSVMaskVisualizer:
    ...
    def draw(self, left_image, right_image):
        ...
        if left_output is not None and self.left_mask is not None:
            left_output = apply_mask_overlay(
                left_output, 
                self.left_mask, 
                alpha=0.3,
            ...
            if left_coords:
                left_output = draw_centroid(left_output, (left_coords[0], left_coords[1]))
        
        if right_output is not None and self.right_mask is not None:
            right_output = apply_mask_overlay(
                right_output, 
                self.right_mask, 
                alpha=0.3,
                hsv_settings=self.hsv_settings
            ...
            if right_coords:
                right_output = draw_centroid(right_output, (right_coords[0], right_coords[1]))
